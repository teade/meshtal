...
//!
//! All parsers for the mcnp module are collected here for re-use across various
//! module that may have common parser needs.
//!
//! Built on the nom crate and broadly falling into either parser combinators
//! or boolean checks. All result and errors are confined to the [nom::IResult]
//! ecosystem within this module.
//!
//! Example
//!
//! ```ignore
//! # use meshtal::readers::parsers;
//! # use meshtal::mesh::Voxel;
//! let line = " Mesh Tally Number       134";
//! assert_eq!(parsers::is_new_mesh(line), true);
//!
//! let line = "1.111E+00    -0.500    -0.733    -2.625 7.25325E-03 1.20187E-02";
//! let voxel = parsers::column_type_voxel(line)?;
//! assert_eq!(voxel, Voxel{index: 0, result: 7.25325E-03, error: 1.20187E-02});
//! ```
//!

// internal modules
use crate::mesh::{Geometry, Group, Voxel};
use crate::point::Point;
use crate::readers::meshtal_file::{CellData, VoidRecord};
use crate::utils::*;
...
/// Parse any number/`Total` to the appropriate [Group] variant
fn point_group(i: &str) -> IResult<&str, Group> {
    let (i, group) = opt(alt((total_group, double_group)))(i)?;
    match group {
        Some(g) => Ok((i, g)),
        None => Ok((i, Group::Total)),
    }
...
pub fn is_points_file_comment(i: &str) -> bool {
    i.starts_with('#')
}

/// Parse keywords to the approrpiate [Geometry]
pub fn points_file_keyword(i: &str) -> IResult<&str, Geometry> {
    map(point_type_tag, |t| match t.to_lowercase().as_str() {
        "xyz" | "rec" => Geometry::Rectangular,
        "rzt" | "cyl" => Geometry::Cylindrical,
        _ => unreachable!(),
    })(i)
}

/// Recognise valid keywords such as `xyz` and `cyl`
fn point_type_tag(i: &str) -> IResult<&str, &str> {
    alt((
        tag_no_case("xyz"),
        tag_no_case("rzt"),
        tag_no_case("cyl"),
...
/// Parse a line in the points.txt file to a [Point]
pub fn points_file_point(i: &str) -> IResult<&str, Point> {
    alt((
        points_file_full_point,
        points_file_energy_only,
        points_file_coords_only,
    ))(i)
}

/// Read full (e,t,i,j,k) point
fn points_file_full_point(i: &str) -> IResult<&str, Point> {
    let (i, energy) = point_group(i)?;
    let (i, _) = space1(i)?;
    let (i, time) = point_group(i)?;
    let (i, _) = space1(i)?;
    let (i, i_coord) = double(i)?;
    let (i, _) = space1(i)?;
    let (i, j_coord) = double(i)?;
...
/// Read (e,i,j,k) point, assume t group is Total
fn points_file_energy_only(i: &str) -> IResult<&str, Point> {
    let (i, energy) = point_group(i)?;
    let (i, _) = space1(i)?;
    let (i, i_coord) = double(i)?;
    let (i, _) = space1(i)?;
...
/// Read (i,j,k) point, assume e and t groupa are Total
fn points_file_coords_only(i: &str) -> IResult<&str, Point> {
    let (i, i_coord) = double(i)?;
    let (i, _) = space1(i)?;
    let (i, j_coord) = double(i)?;
    let (i, _) = space1(i)?;
